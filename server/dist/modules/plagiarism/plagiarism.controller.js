...
exports.plagiarismController = exports.PlagiarismController = void 0;
...
class PlagiarismController {
    async runCheck(req, res, next) {
        ...
        if (project.documents.length === 0) {
            return next(new ApiError_1.ApiError(400, "No document found to check similarity."));
        }
        const latestDoc = project.documents[0];
        // Run check asynchronously
        setImmediate(async () => {
            await similarity_service_1.default.checkProjectSimilarity(projectId, latestDoc.id);
        });
    ...
    async getReport(req, res, next) {
        ...
        if (!report)
            return next(new ApiError_1.ApiError(404, "No similarity report found for this project."));
    ...
    async downloadReport(req, res, next) {
        ...
        if (!report)
            return next(new ApiError_1.ApiError(404, "Similarity report not found."));
        if (format === "json") {
            const reportData = {
...
<body>
  <div class="container">
    ...
    async getMatches(req, res, next) {
        ...
        const matches = await database_1.default.similarityMatch.findMany({
            where: { reportId },
            include: {
                matchedProject: {
                    select: {
...
exports.plagiarismController = new PlagiarismController();
exports.default = exports.plagiarismController;
